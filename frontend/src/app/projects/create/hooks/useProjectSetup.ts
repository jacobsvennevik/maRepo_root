...
export const useProjectSetup = (initialSetup: ProjectSetup) => {
  ...
  const handleOptionSelect = (field: keyof ProjectSetup, value: string | string[]) => {
  ...
  const handleAddDate = (newDate: { date: string; description: string; type: string }) => {
  ...
  const handleApplyAITestTypes = (types: string[]) => {
    ...
    setSetup(prev => ({
      ...prev,
      evaluationTypes: [...new Set([...prev.evaluationTypes, ...convertedTypes])]
    }));
    setHasUnsavedChanges(true);
  };
