...
export function ProjectSummaryDemo({ setup, onBack }: { setup: ProjectSetup; onBack: () => void }) {
          ...
          <div className="pb-4">
            <div className="flex items-center space-x-3">
              <Badge variant="secondary" className="flex items-center space-x-1">
                {(() => {
                  const currentVar = variants.find(v => v.id === currentVariant);
                  const Icon = currentVar?.icon || Monitor;
                  return <Icon className="h-3 w-3" />;
                })()}
                <span>{variants.find(v => v.id === currentVariant)?.name}</span>
              </Badge>
...
export function VariantUsageGuide() {
  return (
    <div className="max-w-4xl mx-auto p-6">
      <h1 className="text-3xl font-bold text-gray-900 mb-6">Project Summary Variants</h1>
      
      <div className="prose prose-lg text-gray-600 mb-8">
        <p>
          Here are 4 different design approaches for the project summary component, 
          each with its own personality and use case:
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {[
          {
            name: "Original",
            description: "Clean, professional design with good information hierarchy",
            useCase: "Perfect for academic institutions and professional environments",
            features: ["Clean typography", "Organized cards", "Professional colors", "Clear structure"],
            preview: "bg-gradient-to-br from-blue-50 to-indigo-50"
          },
          {
            name: "Colorful Dashboard",
            description: "Vibrant, energetic design with animations and bright gradients",
            useCase: "Great for younger audiences and creative subjects",
            features: ["Animated elements", "Bright color scheme", "Visual hierarchy", "Engaging interactions"],
            preview: "bg-gradient-to-br from-purple-50 to-pink-50"
          },
          {
            name: "Glass Morphism",
            description: "Modern dark theme with glass effects and floating elements",
            useCase: "Appeals to tech-savvy users and modern applications",
            features: ["Glass blur effects", "Dark theme", "Floating animations", "Premium feel"],
            preview: "bg-gradient-to-br from-indigo-900 to-purple-900"
          },
          {
            name: "Gamified",
            description: "RPG-style interface with levels, achievements, and progress bars",
            useCase: "Perfect for making learning fun and engaging",
            features: ["Achievement system", "Progress tracking", "RPG aesthetics", "Motivation focus"],
      ...
      <div className="mt-8 p-6 bg-blue-50 rounded-xl">
        ...
        <div className="space-y-2 text-sm text-gray-600">
          <p>1. Import the variant you want to use from `./project-summary-variants`</p>
          <p>2. Replace the `ProjectSummary` component with your chosen variant</p>
