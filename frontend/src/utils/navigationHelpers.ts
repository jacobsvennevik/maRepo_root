 ...
 */
export function isLinkActive(pathname: string, href: string, exact: boolean = false): boolean {
  if (exact) {
...
export function isInProjectContext(pathname: string): boolean {
  return pathname.startsWith('/projects/') && extractProjectIdFromPath(pathname) !== null
}
